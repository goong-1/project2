    ...
    def vision_callback(self, msg):
        ...
        if self.state == "CRUISE" and not crosswalk_detected:
            ...
            # 2순위: 정지선
                ...
                if current_time - self.red_line_lost_time >= 0.6:
        ...
        # 장애물 회피 기동
            ...
            if self.avoid_substate == 1:
                if not self.waiting_for_esp:
                    self.send_command(f"T{-45 * self.avoid_direction}")  # ← 부호 반전
                ...
                self.get_logger().info(f"장애물 회피 완료 → {new_lane}차선으로 변경")
                self.state          = "CRUISE"
                self.avoid_substate = 1
                self.prev_error     = 0.0
                self.integral       = 0.0
            ...
            turn_deg = max(-5, min(5, turn_deg))  # 클램핑 범위도 확대

            if abs(turn_deg) < 2:   # 절댓값으로 판단
                self.send_command(f"G{self.current_speed}")
            else:
