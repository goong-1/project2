...
class CameraParserNode(Node):
    def __init__(self):
        ...
        self.roi_ratio = (0.001, 0.200, 0.999, 0.850) # ROI 비율 (왼상 0.1%, 오른하 70%)
       
        # 라벨별 최소 면적 비율 (클수록 더 가까이 와야 인식)
        # area_ratio = 박스크기 / 화면크기 → 가까울수록 큼
        ...
        self.default_min_area = 0.001   # 목록에 없는 라벨용

        self.last_sent_labels = None
    ...
    def image_callback(self, msg):
        ...
        labels = sorted(set(merge.get(d.label, d.label) for d in detections))
        current = ",".join(labels)
                           
         # ── 직전과 같으면 안 보냄, 바뀌었을 때만 한 번 ──
        if current != self.last_sent_labels:
            out = String()
            out.data = current
            self.result_pub.publish(out)
            self.last_sent_labels = current
            self.get_logger().info(f'[YOLO] 라벨 전송: {current}')

        debug_frame = self.make_debug_frame(frame, detections, f"SEEN: {current}")
        self.publish_yolo_debug_image(debug_frame)
