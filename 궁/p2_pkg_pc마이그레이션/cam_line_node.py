#!/usr/bin/env python3
"""
[PC 전용] cam_line_node.py

원본 대비 변경점:
  - Image 구독 → CompressedImage 구독
  - JPEG 디코드 추가
  - 디버그 이미지 토픽: image_line/compressed (yolo 토픽과 분리)
  - 발행 토픽 유지: /vision_status, /image_line/compressed
"""
...
class VisionNode(Node):
    def __init__(self):
        ...
        self.image_sub = self.create_subscription(
            ...
            self.image_callback, be_qos
        )
        self.state_sub = self.create_subscription(
            String, '/control_state', self.state_callback, 10
        )

        # ── 발행 ──
        self.vision_pub = self.create_publisher(String, '/vision_status', 10)
        self.line_image_pub = self.create_publisher(
            CompressedImage, 'image_line/compressed', be_qos
        )
        ...
        self.get_logger().info(' 차선 감지 노드 시작 (PC 측)              ')
        self.get_logger().info('   구독: /image_raw/compressed            ')
        self.get_logger().info('   발행: /vision_status                   ')
        self.get_logger().info('   발행: /image_line/compressed           ')
        self.get_logger().info('==========================================')
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            self.get_logger().error(f'압축 이미지 송신 실패: {e}')
