...
class ControlHandler(Node):
    def __init__(self):
        ...
        # ★ [추가] 차선 감지 노드가 죽어있을 때 임시 데이터를 주입하는 0.1초 타이머
        self.standalone_timer = self.create_timer(0.1, self.standalone_tick)

    # ★ [추가] 차선 감지 신호가 없어도 기본 직진(115) 및 YOLO 구동을 유지하는 함수
    def standalone_tick(self):
        # 차선 데이터가 1초 이상 안 들어오면 가상의 '완벽한 직진' 데이터를 주입
        if time.time() - self.last_vision_time > 1.0:
            dummy_msg = String()
            dummy_msg.data = "0.0|0|0|0|0|0|0|DUMMY"
            self.vision_callback(dummy_msg)
    ...
    def monitor_status(self):
        ...
        # [수정] 도배 방지를 위해 죽었을 때 1번, 다시 살아났을 때 1번만 경고/안내를 띄움
        if vision_delayed and not self.vision_warned:
            self.get_logger().warning("⚠️ [신호 경고] 차선 감지(Line) 정지 -> 기본 직진(115) 및 YOLO 의존 모드 실행")
            self.vision_warned = True
        elif not vision_delayed and self.vision_warned:
            self.get_logger().info("✅ [신호 복구] 차선 감지(Line) 신호가 다시 들어옵니다.")
            self.vision_warned = False

        if yolo_delayed and not self.yolo_warned:
            self.get_logger().warning("⚠️ [신호 경고] YOLO 신호 대기중 -> YOLO 무시 및 차선 추적 의존 모드 실행")
            self.yolo_warned = True
        elif not yolo_delayed and self.yolo_warned:
            self.get_logger().info("✅ [신호 복구] YOLO 신호가 활성화되었습니다.")
            self.yolo_warned = False

        if not vision_delayed or not yolo_delayed:
    ...
    def vision_callback(self, msg):
        # [수정] DUMMY 데이터인지 판별하여 실제 통신 시간(last_vision_time) 처리
        is_dummy = msg.data.endswith("|DUMMY")
        if not is_dummy:
            self.last_vision_time = time.time()
        ...
        try:
            if is_dummy:
                data = msg.data.replace("|DUMMY", "").split('|')
            else:
                data = msg.data.split('|')
