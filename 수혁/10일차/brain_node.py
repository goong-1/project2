    ...
    def vision_callback(self, msg):
        ...
        if self.state == "CRUISE" and not crosswalk_detected:
            ...
            # 3순위: LiDAR 전방 장애물 감지
                ...
                self.get_logger().warn("라이다(LiDAR) 전방 장애물 감지 -> 정지 후 3초 동적/정적 판별 대기")
                self.state = "OBSTACLE_WAIT"
                self.obstacle_wait_start_time = current_time
                self.avoid_direction = avoid_direction  # 현재 차선 정보를 바탕으로 회피할 방향 고정
            # 4순위: YOLO 좌회전
        ...
        # ★ 장애물 3초 판별 대기
            ...
            if not self.lidar_obstacle_detected:
                self.get_logger().info("장애물 사라짐 (동적 장애물 판단) -> 직진 주행 재개")
                self.state = "CRUISE"
                self.prev_error = 0.0
                self.integral = 0.0
                self.prev_time = time.time()
        ...
        # ★ YOLO 하드코딩 좌회전 시퀀스 (안정화된 ESP 대기 로직 적용)
        ...
        # 장애물 회피 기동
            ...
            if self.avoid_substate == 1:
                if not self.waiting_for_esp:
                    # avoid_direction은 OBSTACLE_WAIT 진입 시점에 이미 저장되어 있음
